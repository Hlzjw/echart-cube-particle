...
import { isRadianAroundZero, remRadian } from 'echarts/lib/util/number';
import { createSymbol, normalizeSymbolOffset } from 'echarts/lib/util/symbol';
import * as matrixUtil from 'zrender/lib/core/matrix';
import { applyTransform as v2ApplyTransform } from 'zrender/lib/core/vector';
import { shouldShowAllLabels } from 'echarts/lib/coord/axisHelper';
var PI = Math.PI;
/**
...
function buildAxisLabel(group, transformGroup, axisModel, opt) {
  ...
  var emphasisModel = labelModel.getModel(['emphasis']);
  var textNumber = labelModel.get('textNumber');
  ...
  var labelEls = [];
  var silent = !(textNumber && textNumber !== 'auto');
  ...
  each(labels, function (labelItem, index) {
    ...
    // enableHoverEmphasis(textEl, emphasisModel.get('focus'), emphasisModel.get('blurScope'));
    var stats = textEl.ensureState('emphasis');
    stats.style = createTextStyle(emphasisModel);

    if (textNumber && textNumber !== 'auto') {
        graphic.setTooltipConfig({
            el: textEl,
            componentModel: itemLabelModel,
            itemName: rawLabel,
    ...
    if (axis.type === 'category' && labelRotation !== 0) {
        const bound = textEl.getBoundingRect();
        if (axis.dim === 'y') {
            const h = bound.height/2;
            const y = h * Math.sin(labelRotation / Math.PI);
            const x = h * Math.cos(labelRotation / Math.PI);
