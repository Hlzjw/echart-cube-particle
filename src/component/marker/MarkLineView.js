...
import * as markerHelper from 'echarts/lib/component/marker/markerHelper';
import MarkerView from 'echarts/lib/component/marker/MarkerView';
import { getStackedDimension } from 'echarts/lib/data/helper/dataStackHelper';
import { isCoordinateSystemType } from 'echarts/lib/coord/CoordinateSystem';
import { getECData } from 'echarts/lib/util/innerStore';
import MarkerModel from 'echarts/lib/component/marker/MarkerModel';
import { isArray, retrieve, retrieve2, clone, extend, logError, merge, map, curry, filter } from 'zrender/lib/core/util';
import { makeInner } from 'echarts/lib/util/model';
import { getVisualFromData } from 'echarts/lib/visual/helper';
...
const markLineTransform = function (seriesModel, coordSys, mlModel, item) {
  ...
  const normalizedItem = [markerHelper.dataTransform(seriesModel, itemArray[0]), markerHelper.dataTransform(seriesModel, itemArray[1]), extend({}, itemArray[2])]; // Avoid line data type is extended by from(to) data type
...
(function (_super) {
  ...
  MarkLineView.prototype.updateTransform = function (markLineModel, ecModel, api) {
    ecModel.eachSeries(function (seriesModel) {
      ...
      if (mlModel) {
        ...
        const fromData_1 = inner(mlModel).from;
        const toData_1 = inner(mlModel).to; // Update visual and layout of from symbol and to symbol
  ...
  MarkLineView.prototype.renderSeries = function (seriesModel, mlModel, ecModel, api) {
    ...
    lineData.each((idx) => {
      ...
      if (lineStyle.innerColor == null) {
        lineStyle.innerColor = lineData.getItemModel(idx).getModel('lineStyle').get('innerColor');
      }
    ...
    mlData.line.eachItemGraphicEl((el, idx) => {
      el.traverse((child) => {
...
function createList(coordSys, seriesModel, mlModel) {
  ...
  if (coordSys) {
    coordDimsInfos = map(coordSys && coordSys.dimensions, (coordDim) => {
  ...
  fromData.initData(map(optData, (item) => item[0]), null, dimValueGetter);
  toData.initData(map(optData, (item) => item[1]), null, dimValueGetter);
  lineData.initData(map(optData, (item) => item[2]));
